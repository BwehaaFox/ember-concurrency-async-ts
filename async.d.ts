...
declare module 'ember-concurrency-ts' {
  function taskFor<T extends AsyncTaskFunction>(task: T): AsyncTaskFor<T, T>;
  function taskFor<T extends AsyncDescriptor>(task: T): AsyncTaskForDescriptor<T>;

  function perform<T extends AsyncTaskFunction>(task: T, ...args: Args<T>): AsyncInstanceFor<T, T>;
